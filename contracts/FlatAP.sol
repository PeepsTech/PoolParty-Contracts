...
contract AaveParty is ReentrancyGuard {
    ...
    *****************/
    function submitProposal(
        ...
        else if (flagNumber == 7) { // for amend governance use tributeOffered for partyGoal, paymentRequested for depositRate, tributeToken for new aToken, paymentToken for new aave
    ...
    function processProposal(uint256 proposalIndex) public nonReentrant {
        ...
        if (didPass) {
            ...
            // burn aToken from local guild balance and deposit underlying to applicant guild account
    ...
    function processAmendGovernance(uint256 proposalIndex) public nonReentrant {
        ...
        bool didPass = _didPass(proposalIndex);
            ...
            if(proposal.tributeToken != address(0)){
                ...
                IERC20(proposal.tributeToken).approve(aave, uint256(-1)); // max approve aave for deposit into aToken
            }
    ...
    function _ragequit(address memberAddress, uint256 sharesToBurn, uint256 lootToBurn) internal {
        ...
        for (uint256 i = 0; i < approvedTokens.length; i++) {
            uint256 amountToRagequit = fairShare(IERC20(aTokenAssignments[approvedTokens[i]]).balanceOf(address(this)), sharesAndLootToBurn, initialTotalSharesAndLoot);
    ...
    ***************/
    function makeDeposit(uint256 amount) external nonReentrant {
        ...
        IAaveDepositWithdraw(aave).deposit(depositToken, amount, address(this), 0); // deposit to aave - return aToken
    }
